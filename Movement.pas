...
{ Goes through the trackcircuits cleared list, adding up the lengths of the sections that are still marked as occupied, to see if any can be released, if the total exceeds
...
BEGIN
  ...
  WITH T^ DO BEGIN
    { First run through the list, removing earlier occurrences of occupation - it may well be that we are re-occupying a circuit on a later occasion - if we don't remove
      the record of the earlier releases, the latest occupation is disregarded
    }
...
{ Makes Thunderbird stop for a minimum time before changing direction **** }
BEGIN
  Result := True;
  WITH T^ DO BEGIN
...
PROCEDURE FindFirstTwoOccurrencesInStringArray(StrToFind : String; StringArray : StringArrayType;
                                               VAR FirstOccurrencePos, SecondOccurencePos : Integer);
{ Find the first and second occurrences of a given string in a string array - return -1 if not found }
VAR
  I : Integer;
...
BEGIN
  ...
  DO BEGIN
    ...
    Inc(I);
  END; {WHILE}
END; { FindFirstTwoOccurrencesInStringArray }
    ...
    WITH T^ DO BEGIN
      IF Length(Train_TCsAndSignalsNotClearedArray) > -1 THEN BEGIN
        ...
        IF CurrentTC <> Train_SaveCurrentTC THEN BEGIN
          ...
          { Now work out how far away the next signals are - this needs to be recalculated each time the current trackcircuit changes; if the signal isn't at the start of
            the array, calculate the distance to it, but ignore the first trackcircuit, as we may already be in that.
          }
        ...
        { Now continue processing whether we're in a new trackcircuit or not - a signal may change while we're traversing one. Our speed depends on the distance to next
          red aspect, if it's fairly close (i.e. YY, Y, R) or at a station we have to stop at it even if the adjacent aspect is not red. Set the minimum speed - bear in
          mind that scale mph and real mph are different - when speed-checking locos, it is the case that the minimum speed for most locos (i.e. before stalling) is 30mph.
        }
        SpeedCalculationStr := '';
        StopStr := '';
...
BEGIN
  WITH T^ DO BEGIN
    IF Train_CurrentStatus <> ReadyForRemovalFromDiagrams THEN BEGIN
      ...
      { If the train isn't going anywhere, don't process any movement - without this check, another train entering the next trackcircuit would be assumed by the system to
        be our loco.
      }
      ...
      IF (Train_CurrentStatus = Missing) OR (Train_CurrentStatus = MissingAndSuspended) THEN BEGIN
        ...
        IF NOT Train_MissingMessage THEN BEGIN
          ...
          { Arrange for it to appear again when the user is ready - perhaps by having redirected it to the correct track, or by making sure that power is reaching the chip }
...
{ Returns true if a given train exists }
VAR
  DebugStr : String;
