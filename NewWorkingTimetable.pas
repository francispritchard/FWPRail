UNIT NewWorkingTimetable;

INTERFACE

USES Windows, Messages, Variants, Classes, Graphics, Controls, Forms, StdCtrls, Grids, Menus, InitVars, GetTime, Dialogs, SysUtils, DB,
     ADODB, DBGrids, ExtCtrls, DBCtrls;
...
USES ComObj, Lenz, MiscUtils, Startup, LocoUtils, IDGlobal, RailDraw, Input, Movement, CreateRoute, DateUtils, Math {sic}, Route, Types,
     StrUtils, StationMonitors, Locks, LocoDialogue, LocationData, Help, Options, Diagrams;
...
{ We identify which items we want to be highlighted or marked as invalid by prefixing them with a marker; this is necessary as this
  routine just draws the data it's given without knowing where that data has come from).
}
CONST
  CharOffset = 3;
...
VAR
  ...
  PROCEDURE SetUpWorkingTimetableGridCell(Status : WorkingTimetableStatusType; WorkingTimetableCol, WorkingTimetableRow : Integer;
                                          Str : String; Style : CellStyleType; Alignment : TextAlignment);
  { May add an cancelled train marker or highlight some text before passing to the DrawWorkingTimetableGridCell routine (this would not
    be necessary if that routine knew where the text was coming from).
  }
  BEGIN
...
BEGIN
      ...
      WHILE WorkingTimetableCount <= High(WorkingTimetableRecArray) DO BEGIN
        WITH WorkingTimetableRecArray[WorkingTimetableCount] DO BEGIN
          ...
          THEN BEGIN
            ...
            { Draw the cells, If we want a cell to be in bold, etc., add <B> - the OnDrawCell routine will substitute Bold text for it
              (there is no easier way to do this, as the OnDrawCell routine doesn't know anything about the antecedents of the text it's
              been given).
            }
            IF WorkingTimetable_LocoChip <> UnknownLocoChip THEN
              SetUpWorkingTimetableGridCell(WorkingTimetable_Status, LocoCol, TempGridRowCount,
                                            LocoChipToStr(WorkingTimetable_LocoChip), NormalStyle, CentreAlign);
            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, EntryCol, TempGridRowCount, WorkingTimetable_EntryNumStr,
                                          NormalStyle, CentreAlign);
            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, FromCol, TempGridRowCount,
                                          AreaToStr(WorkingTimetable_FirstStationArea), NormalStyle, LeftAlign);
            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, FromTimeCol, TempGridRowCount,
                                          TimeToHMSStr(WorkingTimetable_FirstStationDepartureTime), NormalStyle, LeftAlign);
            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, ToCol, TempGridRowCount, AreaToStr(WorkingTimetable_LastStationArea),
                                          NormalStyle, LeftAlign);
            TempStr := '';
            ...
            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, TypeCol, TempGridRowCount,
                                          TrainTypeNumToStr(WorkingTimetable_TrainTypeNum), NormalStyle, LeftAlign);
            TempStr := '';
            IF Length(WorkingTimetable_PossibleLocoClasses) > 0 THEN BEGIN
              ...
              SetUpWorkingTimetableGridCell(WorkingTimetable_Status, PossibleClassesCol, TempGridRowCount, TempStr, NormalStyle,
                                            CentreAlign);
            END;

            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, PreferredLengthCol, TempGridRowCount,
                                          IntToStr(WorkingTimetable_PreferredNumberOfCarriages), NormalStyle, CentreAlign);
            SetUpWorkingTimetableGridCell(WorkingTimetable_Status, DaysOfTheWeekCol, TempGridRowCount,
                                          DaysOfTheWeekSetToStr(WorkingTimetable_DaysOfTheWeekSet), NormalStyle, CentreAlign);
  ...
  EXCEPT
    ON E : Exception DO
      Log(NGoLocoChip, 'E DrawWorkingTimetable: ' + E.ClassName +' error raised, with message: ' + E.Message);
  END; {TRY}
...
BEGIN
  WITH WorkingTimetableWindow DO BEGIN
    ...
    WITH WorkingTimetableWindowGrid DO BEGIN
      ...
      TotalWidthOfColumns := LocoColWidth + EntryColWidth + FromColWidth + FromTimeColWidth + ToColWidth + TypeColWidth
                             + StoppingAtColWidth
                             + PossibleClassesColWidth + PreferredLengthColWidth + DaysOfTheWeekColWidth
...
BEGIN
  CASE Key OF
    ...
    vk_Numlock, vk_Scroll, vk_Cancel { Ctrl-Break}, vk_Capital { Caps Lock }, vk_Back { Backspace }:
      { do nothing };
  ELSE
...
BEGIN
  ...
  OR (NOT LargeWorkingTimetableWindowSelected AND (WorkingTimetableWindow.Width <> DefaultWorkingTimetableSmallWindowWidth))
  OR (LargeWorkingTimetableWindowSelected AND (WorkingTimetableWindow.Width <> DefaultWorkingTimetableLargeWindowWidth))
...
PROCEDURE TWorkingTimetableWindow.WorkingTimetableWindowGridMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
                                                                      X, Y: Integer);
VAR
...
BEGIN
  WITH WorkingTimetableWindow.WorkingTimetableWindowGrid DO BEGIN
    ...
    IF (ColsAndRows.X > -1) AND (ColsAndRows.Y > -1) AND (SaveCursor <> crHandPoint) THEN
    ...
    WITH WorkingTimetableWindow DO BEGIN
      ...
      TRY
        ...
        WorkingTimetableADOConnection.ConnectionString := 'Provider=Microsoft.Jet.OLEDB.4.0; Data Source='
                                                          + PathToRailDataFiles + WorkingTimetableFilename
                                                                                                  + '.' + WorkingTimetableFilenameSuffix
        ...
        WHILE WorkingTimetableOK AND NOT WorkingTimetableWindow.WorkingTimetableADOTable.EOF DO BEGIN
              ...
              WITH WorkingTimetableRecArray[High(WorkingTimetableRecArray)] DO BEGIN
                ...
                IF ErrorMsg = '' THEN BEGIN
                  ...
                  IF ErrorMsg = '' THEN BEGIN
                    ...
                    I := 0;
                    WHILE (I <= High(WorkingTimetable_StoppingAtStationAreas)) AND (ErrorMsg = '') DO BEGIN
                      IF WorkingTimetable_StoppingAtStationAreas[I] = WorkingTimetable_FirstStationArea THEN
                        ErrorMsg := 'Cannot have first station area ' + AreaToStr(WorkingTimetable_FirstStationArea)
                                    + ' as an intermediate station area too'
                      ELSE
                        IF WorkingTimetable_StoppingAtStationAreas[I] = WorkingTimetable_LastStationArea THEN
                          ErrorMsg := 'Cannot have last station area ' + AreaToStr(WorkingTimetable_LastStationArea)
                                      + ' as an intermediate station area too';
                      Inc(I);
                    END; {WHILE}
                  END;
                ...
                IF ErrorMsg = '' THEN BEGIN
                  ...
                  IF TempStr <> '' THEN BEGIN
                    ...
                    FOR I := 0 TO High(WorkingTimetable_PossibleLocoClasses) DO BEGIN
                      T := TrainList;
                      ClassFound := False;
                      WHILE (T <> NIL) AND NOT ClassFound DO BEGIN
                ...
                END ELSE BEGIN
                  DebugStr := 'W=' + WorkingTimetable_EntryNumStr + ': '
                               + AreaToStr(WorkingTimetable_FirstStationArea) + ' -> ' + AreaToStr(WorkingTimetable_LastStationArea)
                               + ' ' + TimeToHMStr(WorkingTimetable_FirstStationDepartureTime)
                               + ' ' + TrainTypeNumToStr(WorkingTimetable_TrainTypeNum);
      ...
      EXCEPT {TRY}
        ON E : Exception DO
          Log(NGoLocoChip, 'E InitialiseWorkingTimetable: ' + E.ClassName +' error raised, with message: '+ E.Message);
      END; {TRY}
    END; {WITH}
...
BEGIN
  ...
  WITH W DO BEGIN
    ...
    Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
           + 'maximum platform length at the first station is ' + RealToStr(Result));
      ...
      FOR Location := 0 TO High(Locations) DO BEGIN
        IF Locations[Location].Location_Area = WorkingTimetable_StoppingAtStationAreas[MiddleStationCount] THEN BEGIN
          ...
          IF Locations[Location].Location_PlatformOrFiddleyardAtUp <> UnknownLocation THEN BEGIN
            IF Locations[Location].Location_LengthInInches
                             + Locations[Locations[Location].Location_PlatformOrFiddleyardAtUp].Location_LengthInInches > TempResult
            THEN
              TempResult  := Locations[Location].Location_LengthInInches
                             + Locations[Locations[Location].Location_PlatformOrFiddleyardAtUp].Location_LengthInInches;
          END;

          IF Locations[Location].Location_PlatformOrFiddleyardAtDown <> UnknownLocation THEN BEGIN
            IF Locations[Location].Location_LengthInInches
                             + Locations[Locations[Location].Location_PlatformOrFiddleyardAtDown].Location_LengthInInches > TempResult
            THEN
              TempResult  := Locations[Location].Location_LengthInInches
                             + Locations[Locations[Location].Location_PlatformOrFiddleyardAtDown].Location_LengthInInches;
          END;
        END;
      END; {FOR}
      IF TempResult < Result THEN BEGIN
        Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
               + 'maximum platform length at intermediate station '
               + AreaToStr(WorkingTimetable_StoppingAtStationAreas[MiddleStationCount])
               + ' is ' + RealToStr(TempResult)
               + ' which is less than the max platform length ' + RealToStr(Result) + ' so far');
        Result := TempResult;
      END;
    END; {FOR}
    ...
    IF TempResult = 0 THEN BEGIN
      Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
             + 'platform length for ' + AreaToStr(WorkingTimetable_LastStationArea) + ' is zero! - entry cancelled');
      WorkingTimetable_Status := EntryCancelled;
    END ELSE
      IF TempResult < Result THEN BEGIN
        Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
               + 'maximum platform length at last station ' + AreaToStr(WorkingTimetable_LastStationArea)
               + ' is ' + RealToStr(TempResult)
               + ' which is less than the max platform length ' + RealToStr(Result) + ' at the previous stations');
        Result := TempResult;
      END;
  END; {WITH}
END; { GetRequiredStationsMaxLength }

FUNCTION GetAreaToAreaRouteLengthInInches(EntryNumStr : String; Area1, Area2, TrainTypeNum : Integer;
                                          Direction : DirectionType; TrainLength : Integer) : Real;
...
BEGIN
  ...
  TRY
    ...
    WHILE (Location <= High(Locations)) AND NOT SuitableLocationFound DO BEGIN
      IF Locations[Location].Location_Area = Area1 THEN BEGIN
        ...
        WHILE (TempLocationsCount <= High(TempLocationsArray)) AND NOT SuitableLocationFound DO BEGIN
    ...
    IF OK THEN BEGIN
      ...
      FindRouteFromLineAToLineB(NoLocoChip, NoJourney, UnknownSignal, StartLine, EndLine, Direction,
                                TrainTypeNumToTrainType(TrainTypeNum), TrainLength, UseEmergencyRouteing, NOT IncludeOutOfUseLines,
                                RouteArray, LinesNotAvailableStr, OK);
      IF NOT OK THEN
        Log('W W=' + EntryNumStr + ':    '
               + 'no route found between ' + LineToStr(StartLine) + ' and ' + LineToStr(EndLine)
               + ' In GetJourneyTimeInMinutesFromRouteLength')
      ELSE
        Result := CalculateRouteLength(RouteArray);
    END;
  EXCEPT
    ON E : Exception DO
      Log(NGoLocoChip, 'E GetAreaToAreaRouteLengthInInches: ' + E.ClassName +' error raised, with message: ' + E.Message);
  END; {TRY}
END; { GetAreaToAreaRouteLengthInInches }

FUNCTION GetJourneyTimeInMinutesFromRouteLength(EntryNumStr : String; StartArea, EndArea, TrainTypeNum : Integer;
                                                RouteLengthInInches : Real; Direction : DirectionType) : Integer;
...
BEGIN
  ...
  Result :=  Result + 1;
  Log('W W=' + EntryNumStr + ':    '
         + 'route ' + DirectionToStr(Direction) + ' between ' + AreaToStr(StartArea) + ' and ' + AreaToStr(EndArea)
         + ' is ' + RealToStr(RouteLengthInInches) + ' inches; time is '
         + IntToStr(Result) + IfThen(Result = 1, ' minute', ' minutes'));
...
BEGIN
  ...
  WHILE (AreasCount <= High(Areas[FirstArea].Area_AccessibleAreasUp)) AND NOT (Result <> UnknownDirection) DO BEGIN
  ...
  IF Result = UnknownDirection THEN BEGIN
    AreasCount := 0;
    WHILE (AreasCount <= High(Areas[FirstArea].Area_AccessibleAreasDown)) AND NOT (Result <> UnknownDirection) DO BEGIN
...
{ See which of the locos are in the right starting place and if they are available at the required departure time for the required
  journey time
}
VAR
  T : Train;
  ...
  WHILE T <> NIL DO BEGIN
    WITH T^ DO BEGIN
      ...
      IF Train_WorkingTimetableLastArrivalArea = UnknownArea THEN BEGIN
        ...
        IF (T^.Train_LastLocation <> UnknownLocation) AND (Locations[T^.Train_LastLocation].Location_Area = Area) THEN BEGIN
        ...
        IF T^.Train_WorkingTimetableLastArrivalArea = Area THEN BEGIN
          IF CompareTime(T^.Train_WorkingTimetableLastArrivalTime, DepartureTime) <= 0 THEN BEGIN
            ...
            Log('W W=' + EntryNumStr + ':      '
                   ...
                   + ' added as its last arrival time of '
                   + TimeToHMSStr(T^.Train_WorkingTimetableLastArrivalTime)
                   + ' at ' + AreaToStr(Area)
                   + ' is equal to or earlier than required departure time of '
                   + TimeToHMSStr(DepartureTime));
          END ELSE
            Log('W W=' + EntryNumStr + ':      '
                   + LocoChipToStr(T^.Train_LocoChip)
                   + ' not added as its last arrival time of '
                   + TimeToHMSStr(T^.Train_WorkingTimetableLastArrivalTime)
                   + ' at ' + AreaToStr(Area)
                   + ' is later than the required departure time of '
                   + TimeToHMSStr(DepartureTime));
        END;
      END;
    END; {WITH}
...
FUNCTION WhichAvailableTrainsAreSuitable(EntryNumStr : String; AllTrains : IntegerArrayType; PossibleLocoClasses : StringArrayType;
                                         MaxLength : Real {; RouteDirection : DirectionType}) : IntegerArrayType;
{ Select those trains match the classes required and which fit the platforms }
  ...
  BEGIN
    ...
    EXCEPT
      ON E : Exception DO
        Log(NGoLocoChip, 'E SortTrainsInIntegerArrayByLength: ' + E.ClassName +' error raised, with message: '+ E.Message);
    END; {TRY}
  ...
  FOR ClassCount := 0 TO High(PossibleLocoClasses) DO BEGIN
    ...
    ELSE BEGIN
      ...
      IF Length(UnsuitableLengthArray) > 0 THEN BEGIN
        ...
        Log('W W=' + EntryNumStr + ':      '
               + 'looking for trains shorter than ' + RealToStr(MaxLength) + ': '
...
//              IF Locations[T^.Train_LastLocation].Location_LineAtUpIsEndOfLine AND (RouteDirection = Up) THEN
//                InaccessibleTrainFound := True
//              ELSE
//                IF Locations[T^.Train_LastLocation].Location_LineAtDownIsEndOfLine AND (RouteDirection = Down) THEN
//                  InaccessibleTrainFound := True;
//          END;
//
...
//            Log('W W=' + EntryNumStr + ':    '
//                   + 'deleting ' + LocoChipToStr(T^.Train_LocoChip)
//                   + ' from PossiblySuitableTrainsArray as the train is inaccessible');
//
//            Dec(TrainCount);
//          END;
//          Inc(TrainCount);
...
BEGIN
  ...
  WHILE (I <= High(SuitableTrains)) AND NOT InUseTrainFound DO BEGIN
...
PROCEDURE InsertAdditionalWorkingTimetableEntry(EntryNumStr : String; SelectedLocoChip, WorkingTimetableCount, StartArea, EndArea,
                                                TravelTimeInMinutes : Integer; DepartureTime : TDateTime; Direction : DirectionType);
{ Insert a new working timetable entry to get us to the point of initial departure. The entry takes some data from the entry it
  precedes.
}
BEGIN
  TRY
...
//      IF Length(WorkingTimetableRecArray) > (WorkingTimetableCount + 1) THEN BEGIN
        WorkingTimetable_PossibleLocoClasses :=
                                               WorkingTimetableRecArray[WorkingTimetableCount + 1].WorkingTimetable_PossibleLocoClasses;
        WorkingTimetable_PreferredNumberOfCarriages :=
                                        WorkingTimetableRecArray[WorkingTimetableCount + 1].WorkingTimetable_PreferredNumberOfCarriages;
...
//      END;
  ...
  EXCEPT
    ON E : Exception DO
      Log(NoLocoChip, 'EG InsertAdditionalWorkingTimetableEntry: ' + E.ClassName +' error raised, with message: ' + E.Message);
  END; {TRY}
END; { InsertAdditionalWorkingTimetableEntry }

PROCEDURE FindAdditionalTrains(EntryNumStr : String; StationsMaxLength : Real; WorkingTimetableCount : Integer;
                               WorkingTimetableRec : WorkingTimetableRecType;
...
TYPE
  ...
  PROCEDURE CheckAccessForAreasWithTrains(EntryNumStr : String; WorkingTimetableRec : WorkingTimetableRecType; EndArea : Integer;
                                          VAR AreasWithTrainsArray : ARRAY OF AreasWithTrainsRecType;
                                          OUT RouteDirection : DirectionType; OUT AllAccessibleAreas : IntegerArrayType);
    ...
    WITH WorkingTimetableRec DO BEGIN
      FOR I := 0 TO High(AreasWithTrainsArray) DO BEGIN
        WITH AreasWithTrainsArray[I] DO BEGIN
          ...
          ELSE BEGIN
            { See if any of the supplied trains are not able to leave in this direction - use the record of the train's last resting
            ...
            WHILE J <= High(AreasWithTrains_SuitableTrains) DO BEGIN
              ...
              IF T^.Train_WorkingTimetableLastArrivalArea = UnknownArea THEN BEGIN
                { We can only use last location if the train hasn't been relocated as part of this process }
                IF T^.Train_LastLocation <> UnknownLocation THEN
                  IF Locations[T^.Train_LastLocation].Location_LineAtUpIsEndOfLine AND (RouteDirection = Up) THEN BEGIN
                    Log('W W=' + EntryNumStr + ':    '
                    ...
                    Dec(J);
                END ELSE
                  IF Locations[T^.Train_LastLocation].Location_LineAtDownIsEndOfLine AND (RouteDirection = Down) THEN BEGIN
                    Log('W W=' + EntryNumStr + ':    '
                           + 'train ' + LocochipToStr(AreasWithTrains_SuitableTrains[J])
...
BEGIN { FindAdditionalTrains }
  WITH WorkingTimetableRec DO BEGIN
    ...
    { First choose those areas where there are potentially suitable trains available half-an-hour before they are required.
      Half-an-hour is chosen at random, though it would probably not be sensible to move trains into position over a much longer
      period than this.
    }
    Log('W W=' + EntryNumStr + 'A: '
         + 'NEW WORKING TIMETABLE ENTRY:'
         + ' checking possible additional journey to ' + UpperCase(AreaToStr(WorkingTimetable_FirstStationArea))
         + ' by means of making one movement');
    ...
    ELSE BEGIN
      ...
      FOR I := 0 TO High(AreasWithTrainsArray) DO
        Log('W W=' + EntryNumStr + 'A:      '
               + AreaToStr(AreasWithTrainsArray[I].AreasWithTrains_Area) + ': '
               + ListLocoChipsInIntegerArray(AreasWithTrainsArray[I].AreasWithTrains_SuitableTrains));

      Log('W W=' + EntryNumStr + 'A: '
             + 'Checking access for areas with trains:');
      CheckAccessForAreasWithTrains(EntryNumStr + 'A', WorkingTimetableRec, WorkingTimetable_FirstStationArea, AreasWithTrainsArray,
                                    TempDirection, AllAccessibleAreas);
          ...
          WITH AreasWithTrainsArray[I] DO BEGIN
            IF AreasWithTrains_AreaAccessible THEN BEGIN
              ...
              WHILE J <= High(AreasWithTrains_SuitableTrains) DO BEGIN
                ...
                Log('W W=' + EntryNumStr + 'A:    '
                       + 'adding ' + LocoChipToStr(SuitableAdditionalTrains[J].SuitableAdditionalTrains_LocoChip)
                       + ' to list of suitable additional trains');
                Inc(J);
              END {WHILE}
            END;
          END; {WITH}
      ...
      END ELSE BEGIN
        ...
        Log('W W=' + EntryNumStr + 'B: '
             + 'NEW WORKING TIMETABLE ENTRY:'
             + ' checking possible additional journey to ' + UpperCase(AreaToStr(WorkingTimetable_FirstStationArea))
             + ' by means of making two movements');
        ...
        ELSE
          ...
          IF (Length(ReversingAreas) = 1) AND (ReversingAreas[1].ReversingAreas_Area = WorkingTimetable_FirstStationArea) THEN
            Log('W W=' + EntryNumStr + 'B:      '
...
//GetNearestReversingAreaToFirstStationArea
            ...
            { Add the time from each reversing area to the first station area here. This cannot be done globally as it will change
              when the first station area changes.
            }
            Log('W W=' + EntryNumStr + 'B: '
              ...
              WITH ReversingAreas[I] DO BEGIN
                IF ReversingAreas_Area <> WorkingTimetable_FirstStationArea THEN BEGIN
                  ReversingAreas_ToFirstStationAreaDirection :=
                                                            GetRouteDirection(ReversingAreas_Area, WorkingTimetable_FirstStationArea);
                ...
                FOR J := 0 TO High(AreasWithTrainsArray) DO BEGIN
                  WITH AreasWithTrainsArray[J] DO BEGIN
                    IF ReversingAreas_Area = AreasWithTrains_Area THEN
                      Log('W W=' + EntryNumStr + 'B:    '
                             + 'First station area ' + AreaToStr(WorkingTimetable_FirstStationArea)
                             + ' is the same as reversing area - excluding it')
                    ELSE BEGIN
            ...
            ELSE BEGIN
              FOR I := 0 TO High(ReversingAreas) DO BEGIN
                ...
                CheckAccessForAreasWithTrains(EntryNumStr + 'B', WorkingTimetableRec, ReversingAreas[I].ReversingAreas_Area,
                                              AreasWithTrainsArray, TempDirection, TempAccessibleAreas);
            ...
            END ELSE BEGIN
              ...
              Log('W W=' + EntryNumStr + 'C: '
                   + 'NEW WORKING TIMETABLE ENTRY:'
                   + ' checking possible additional journey to ' + UpperCase(AreaToStr(WorkingTimetable_FirstStationArea))
                   + ' by means of making three movements');
                ...
                WITH AreasWithTrainsArray[I] DO BEGIN
                    ...
                    WITH ReversingAreas[J] DO BEGIN
                      ...
                      IF ReversingAreas_Area = WorkingTimetable_FirstStationArea THEN
                        Log('W W=' + EntryNumStr + 'C:        '
                               ...
                               + ' as it is also the first station area')
                      ELSE BEGIN
                        ...
                        ELSE BEGIN
                          ...
                          TempLengthInInches := GetAreaToAreaRouteLengthInInches(EntryNumStr + 'B',
                                                                                 ...
                                                                                 WorkingTimetable_PreferredNumberOfCarriages);
                          Log('W W=' + EntryNumStr + 'C:        '
                                 + 'distance to reversing area ' + AreaToStr(ReversingAreas_Area)
                                 + ' is ' + RealToStr(TempLengthInInches) + ' inches');
                          ...
                          IF TempLengthInInches < MinLengthInInches THEN BEGIN
                            ...
                            Log('W W=' + EntryNumStr + 'C:        '
                                   + 'checking whether there''s a route between given reversing area '
                                   + AreaToStr(ReversingAreas_Area)
                                   + ' and the nearest reversing area to the first station '
                                   + AreaToStr(NearestReversingAreaToFirstStationArea));
                            TempDirection := GetRouteDirection(ReversingAreas_Area, NearestReversingAreaToFirstStationArea);
                            IF TempDirection = UnknownDirection THEN
                              Log('W W=' + EntryNumStr + 'C:        '
                                     + 'excluding reversing area ' + AreaToStr(ReversingAreas_Area)
                                     + ': no route to nearest reversing area to first station'
                                     + AreaToStr(NearestReversingAreaToFirstStationArea))
                            ELSE BEGIN
                              ...
                              Log('W W=' + EntryNumStr + 'C:        '
                                     ...
                                     + ' to the nearest reversing area to the first station '
                                     + AreaToStr(NearestReversingAreaToFirstStationArea)
                                     + ' is ' + RealToStr(TempLengthInInches) + ' inches');

                              TempTravelTimeInMinutes := GetJourneyTimeInMinutesFromRouteLength(
                                                                                               EntryNumStr + 'B',
                                                                                               ReversingAreas_Area,
                                                                                               NearestReversingAreaToFirstStationArea,
                                                                                               WorkingTimetable_TrainTypeNum,
                              ...
                              AreasWithTrains_TravelTimeFromFirstReversingAreaToFinalReversingAreaInMinutes :=
                                                                                                              TempTravelTimeInMinutes;
                            END;
                          END;
                  ...
                  IF NearestReversingAreaToAreaWithTrainsFound THEN
                    Log('W W=' + EntryNumStr + 'C:      '
                           ...
                           + '; area with trains direction to it is '
                           + DirectionTostr(AreasWithTrainsArray[I].AreasWithTrains_ToNearestReversingAreaDirection));
                END; {WITH}
              END; {FOR}
              ...
              ELSE BEGIN
                FOR I := 0 TO High(ReversingAreas) DO BEGIN
                  Log('W W=' + EntryNumStr + 'C: '
                         + 'Checking access to reversing area ' + AreaToStr(ReversingAreas[I].ReversingAreas_Area)
                         + ' for areas with trains:');
                  CheckAccessForAreasWithTrains(EntryNumStr + 'C', WorkingTimetableRec, ReversingAreas[I].ReversingAreas_Area,
                                                AreasWithTrainsArray, TempDirection, TempAccessibleAreas);
                  IF Length(TempAccessibleAreas) = 0 THEN BEGIN
                    OK := True;
                    AppendIntegerArray2ToIntegerArray1(AllAccessibleAreas, TempAccessibleAreas);
                      ...
                      WHILE J <= High(AreasWithTrains_SuitableTrains) DO BEGIN
                        ...
                        WITH SuitableAdditionalTrains[High(SuitableAdditionalTrains)] DO BEGIN
                          ...
                          Log('W W=' + EntryNumStr + 'C:    '
                                 + 'adding ' + LocoChipToStr(SuitableAdditionalTrains[J].SuitableAdditionalTrains_LocoChip)
                                 + ' to list of suitable additional trains: '
                                 + AreaToStr(SuitableAdditionalTrains_StartArea, ShortString)
                                 + ' ' + DirectionToStr(SuitableAdditionalTrains_Direction1)
                                 + ' to ' + AreaToStr(SuitableAdditionalTrains_EndArea1, ShortString)
                                 + ' at ' + TimeToHMStr(SuitableAdditionalTrains_DepartureTime1)
                                 + ' (' + IntToStr(SuitableAdditionalTrains_TravelTimeInMinutes1) + ' mins)'
                                 + ' ' + DirectionToStr(SuitableAdditionalTrains_Direction2)
                                 + ' to ' + AreaToStr(SuitableAdditionalTrains_EndArea2, ShortString)
                                 + ' at ' + TimeToHMStr(SuitableAdditionalTrains_DepartureTime2)
                                 + ' (' + IntToStr(SuitableAdditionalTrains_TravelTimeInMinutes2) + ' mins)'
                                 + ' ' + DirectionToStr(SuitableAdditionalTrains_Direction3)
                                 + ' to ' + AreaToStr(SuitableAdditionalTrains_EndArea3, ShortString)
...
PROCEDURE SelectMostSuitableAdditionalTrain(EntryNumStr : String; SuitableAdditionalTrains : SuitableAdditionalTrainsArrayType;
                                            OUT SelectedLocoChip, StartArea, EndArea1, EndArea2, EndArea3 : Integer;
                                            OUT Direction1, Direction2, Direction3 : DirectionType;
                                            OUT TravelTimeInMinutes1, TravelTimeInMinutes2, TravelTimeInMinutes3 : Integer;
                                            OUT DepartureTime1, DepartureTime2, DepartureTime3 : TDateTime;
                                            PreferredNumberOfCarriages : Integer);
...
TYPE
  SuitableTrainsSortType = (ByNumberOfCarriages, ByTrainLength, ByNearestTrain);
    ...
    PROCEDURE SortTrainsInSuitableTrainsArray(EntryNumStr : String; SortType : SuitableTrainsSortType;
                                              VAR SuitableTrainsArray : SuitableAdditionalTrainsArrayType);
    { Sort an array of train locochip numbers by train length }
    VAR
      DebugStr : String;
      I, J, K : Integer;
...
BEGIN { SelectMostSuitableAdditionalTrain }
  ...
  END ELSE BEGIN
    { otherwise choose the one nearest that matches the length preference and is already in use - we have to use a weighting system
...
BEGIN
  WITH WorkingTimetableRec DO BEGIN
    ...
    IF WorkingTimetable_Status = EntryCancelled THEN BEGIN
      OK := False;
      Log('W W=' + WorkingTimetable_EntryNumStr + ': '
             + 'ENTRY CANCELLED - DIAGRAM NOT CREATED');
    END;

    IF OK AND (WorkingTimetable_LocoChip = UnknownLocoChip) THEN BEGIN
      OK := False;
      Log('W W=' + WorkingTimetable_EntryNumStr + ': '
             + 'NO LOCOCHIP SELECTED - DIAGRAM NOT CREATED');
      WorkingTimetable_Status := EntryCancelled;
    END;
    ...
    IF OK THEN BEGIN
      ...
      IF T = NIL THEN BEGIN
        OK := False;
        Log('W W=' + WorkingTimetable_EntryNumStr + ': '
               + 'NO TRAIN RECORD FOR LOCO ' + LocoChipToStr(WorkingTimetable_LocoChip) + ' - DIAGRAM NOT CREATED');
      END ELSE BEGIN
        { Create the diagram }
        Log('W W=' + WorkingTimetable_EntryNumStr + ': '
               + 'CREATING DIAGRAM ENTRY for ' + LocoChipToStr(WorkingTimetable_LocoChip));
        ...
        CreateTrainDiagramsRecord(WorkingTimetable_LocoChip, DoubleHeaderLocoChip, JourneyCount, UserSpecifiedDepartureTimesArray,
                                  LightsOnTime, EndLocationsStrArray, DirectionsArray, LightsRemainOn, TrainNonMoving,
                                  NotForPublicUseArray, StartLocationStr, StoppingArray, LengthOfTrainInCarriages, TypeOfTrainNum,
                                  UserDriving, UserRequiresInstructions, StartOfRepeatJourney);

        Log('W W=' + WorkingTimetable_EntryNumStr + ': '
               +  LocoChipToStr(WorkingTimetable_LocoChip)
               + ' ' + StartLocationStr + ' -> ' + EndLocationsStrArray[High(EndLocationsStrArray)]
               + ' at ' + TimeToHMSStr(UserSpecifiedDepartureTimesArray[0])
               + ' arriving at '
               + TimeToHMSStr(IncMinute(UserSpecifiedDepartureTimesArray[0], WorkingTimetable_TravelTimeInMinutes))
               + IfThen(NotForPublicUseArray[0], ' [Not for Public Use]', '')
               + ' created');
      END;
    END;
...
VAR
  AllTrainsAtFirstStation : IntegerArrayType;
  DebugStr : String;
  DepartureTime1, DepartureTime2, DepartureTime3 : TDateTime;
  Direction1, Direction2, Direction3 : DirectionType;
  EndArea1, EndArea2, EndArea3 : Integer;
  I : Integer;
  ImportTrain : Boolean;
  LastWorkingTimetableCount : Integer;
  OK : Boolean;
  SelectedLocoChip : Integer;
  StartArea : Integer;
  StationsMaxLength : Real;
  SuitableAdditionalTrains : SuitableAdditionalTrainsArrayType;
  SuitableTrainsAtFirstStation : IntegerArrayType;
  T : Train;
  TempDirection1, TempDirection2 : DirectionType;
  TempLength : Real;
  TimetabledMovementTravelTimeInMinutes : Integer;
  TravelTimeInMinutes1, TravelTimeInMinutes2, TravelTimeInMinutes3 : Integer;
  WorkingTimetableCount : Integer;
...
BEGIN { ProcessWorkingTimetable }
  TRY
    ...
    WHILE WorkingTimetableCount <= High(WorkingTimetableRecArray) DO BEGIN
      ...
      DepartureTime2 := 0;
      DepartureTime3 := 0;
      Direction1 := UnknownDirection;
      Direction2 := UnknownDirection;
      Direction3 := UnknownDirection;
      StartArea := UnknownArea;
      EndArea1 := UnknownArea;
      EndArea2 := UnknownArea;
      EndArea3 := UnknownArea;
      SelectedLocoChip := UnknownLocoChip;
      StationsMaxLength := 0;
      TravelTimeInMinutes1 := 0;
      TravelTimeInMinutes2 := 0;
      TravelTimeInMinutes3 := 0;

      DrawLineInLogFile(NoLocoChip, 'W', '~~', UnitRef);

      WITH WorkingTimetableRecArray[WorkingTimetableCount] DO BEGIN
        WorkingTimetable_LocoChip := 0;
        WorkingTimetable_TravelTimeInMinutes := 0;

        IF CurrentRailwayDayOfTheWeek IN WorkingTimetable_DaysOfTheWeekSet THEN BEGIN
          OK := True;
          Log('W W=' + WorkingTimetable_EntryNumStr + ': '
                 + 'entry validated: current day of the week ' + DayOfTheWeekToStr(CurrentRailwayDayOfTheWeek)
                 + ' is in WorkingTimetable_DaysOfTheWeekSet (' + DaysOfTheWeekSetToStr(WorkingTimetable_DaysOfTheWeekSet) + ')');
        END ELSE BEGIN
          OK := False;
          Log('W W=' + WorkingTimetable_EntryNumStr + ': '
                 + 'entry invalidated: current day of the week ' + DayOfTheWeekToStr(CurrentRailwayDayOfTheWeek)
                 + ' is not in WorkingTimetable_DaysOfTheWeekSet (' + DaysOfTheWeekSetToStr(WorkingTimetable_DaysOfTheWeekSet) + ')');
          WorkingTimetable_Status := IncorrectDayOfTheWeekForEntry;
        END;

        IF OK THEN BEGIN
          Log('W W=' + WorkingTimetable_EntryNumStr + ': '
                 + 'NEW WORKING TIMETABLE ENTRY:'
                 + ' checking possible journey from ' + UpperCase(AreaToStr(WorkingTimetable_FirstStationArea))
                 + ' to ' + UpperCase(AreaToStr(WorkingTimetable_LastStationArea))
                 + ' at ' + TimeToHMSStr(WorkingTimetable_FirstStationDepartureTime)
                 + ' using ' + IfThen(Length(WorkingTimetable_PossibleLocoClasses) = 1, 'class', 'classes')
                 + ' ' + StringArrayToStr(WorkingTimetable_PossibleLocoClasses)
                 + ' of preferred length ' + IntToStr(WorkingTimetable_PreferredNumberOfCarriages));

          WorkingTimetable_Direction := GetRouteDirection(WorkingTimetable_FirstStationArea, WorkingTimetable_LastStationArea);
          IF WorkingTimetable_Direction = UnknownDirection THEN BEGIN
            OK := False;
            Log('WG W=' + WorkingTimetable_EntryNumStr + ': '
                   + AreaToStr(WorkingTimetable_LastStationArea)
                   + ' is not accessible from ' + AreaToStr(WorkingTimetable_FirstStationArea));
          END;
        END;
        ...
        IF OK THEN BEGIN
          { Choose a train by looking through the list of available trains to see which ones are suitable. Trains should ideally be in
            place, though it is possible to move them into place from sidings or from the fiddleyard.
          }
          { Work out how long each trip will take }
          WITH WorkingTimetableRecArray[WorkingTimetableCount] DO BEGIN
            TempLength := GetAreaToAreaRouteLengthInInches(WorkingTimetable_EntryNumStr,
                                                           WorkingTimetable_FirstStationArea,
                                                           WorkingTimetable_LastStationArea,
                                                           WorkingTimetable_TrainTypeNum,
                                                           WorkingTimetable_Direction,
                                                           WorkingTimetable_PreferredNumberOfCarriages);

            TimetabledMovementTravelTimeInMinutes := GetJourneyTimeInMinutesFromRouteLength(WorkingTimetable_EntryNumStr,
                                                                                            WorkingTimetable_FirstStationArea,
                                                                                            WorkingTimetable_LastStationArea,
                                                                                            WorkingTimetable_TrainTypeNum,
                                                                                            TempLength,
          ...
          { And add time for stopping in stations }
          IF Length(WorkingTimetable_StoppingAtStationAreas) > 0 THEN BEGIN
            Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
                   + IntToStr(StationSameDirectionExitMinimumWaitTimeInMinutes * Length(WorkingTimetable_StoppingAtStationAreas))
                   + ' minute added to TimetabledMovementTravelTimeInMinutes ('
                   + IntToStr(TimetabledMovementTravelTimeInMinutes) + ')');
            TimetabledMovementTravelTimeInMinutes := TimetabledMovementTravelTimeInMinutes
                                 + (StationSameDirectionExitMinimumWaitTimeInMinutes * Length(WorkingTimetable_StoppingAtStationAreas));
            WorkingTimetable_TravelTimeInMinutes := TimetabledMovementTravelTimeInMinutes;
          END;

          { See how long each of the platforms on the route are }
          StationsMaxLength := GetRequiredStationsMaxLength(WorkingTimetableRecArray[WorkingTimetableCount]);
          Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
                 + 'maximum platform length is ' + RealToStr(StationsMaxLength));

          { Ascertain which trains are available }
          AllTrainsAtFirstStation := WhichTrainsAreAvailableAtRequiredTime(WorkingTimetable_EntryNumStr,
                                                                           WorkingTimetable_FirstStationArea,
                                                                           WorkingTimetable_FirstStationDepartureTime);
          IF Length(AllTrainsAtFirstStation) = 0 THEN BEGIN
            OK := False;

            { We will need to import a train from elsewhere }
            ImportTrain := True;
            Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
                   + 'no train selected - there are no trains at first station area ' + AreaToStr(WorkingTimetable_FirstStationArea));
          END ELSE BEGIN
            DebugStr := 'trains at first station area ' + AreaToStr(WorkingTimetable_FirstStationArea) + ' are : ';
            FOR I := 0 TO High(AllTrainsAtFirstStation) - 1 DO
              DebugStr := DebugStr + LocoChipToStr(AllTrainsAtFirstStation[I]) + ', ';
            DebugStr := DebugStr + LocoChipToStr(AllTrainsAtFirstStation[High(AllTrainsAtFirstStation)]);
            Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
                   + DebugStr);

            IF WorkingTimetable_Status <> EntryCancelled THEN BEGIN
              { Select trains that match the specifications }   { may need to test for suitable direction here ***** }
              SuitableTrainsAtFirstStation := WhichAvailableTrainsAreSuitable(WorkingTimetable_EntryNumStr,
                                                                              AllTrainsAtFirstStation,
                                                                              WorkingTimetable_PossibleLocoClasses,
                                                                              StationsMaxLength);
              IF Length(SuitableTrainsAtFirstStation) = 0 THEN BEGIN
                OK := False;

                { We will need to import a train from elsewhere }
                ImportTrain := True;
                Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
                       + 'no train selected - there are no suitable trains at ' + AreaToStr(WorkingTimetable_FirstStationArea)
                       + ' able to depart in the ' + DirectionToStr(WorkingTimetable_Direction) + ' direction');

              END ELSE BEGIN
                DebugStr := 'suitable trains in order of class then length at first station area '
                            + AreaToStr(WorkingTimetable_FirstStationArea) + ': ';
                FOR I := 0 TO High(SuitableTrainsAtFirstStation) - 1 DO
                  DebugStr := DebugStr + LocoChipToStr(SuitableTrainsAtFirstStation[I]) + ', ';
                DebugStr := DebugStr + LocoChipToStr(SuitableTrainsAtFirstStation[High(SuitableTrainsAtFirstStation)]);
                Log('W W=' + WorkingTimetable_EntryNumStr + ':    '
                       + DebugStr);
              END;
            END;
        ...
        { See which of them is currently in use - this means we don't have to warm up or crew unnecessary locos... }
        IF OK THEN BEGIN
          IF Length(SuitableTrainsAtFirstStation) = 1 THEN BEGIN
            SelectedLocoChip := SuitableTrainsAtFirstStation[0];
            Log('W W=' + WorkingTimetable_EntryNumStr + ': '
                   + 'Only one train is suitable so selected loco chip is ' + LocoChipToStr(SelectedLocoChip));
          END ELSE BEGIN
            SelectedLocoChip := GetFirstTrainCurrentlyInUse(WorkingTimetable_EntryNumStr, SuitableTrainsAtFirstStation);

            { ...but if none are currently in use, choose the first, already selected at random }
            IF SelectedLocoChip = UnknownLocoChip THEN BEGIN
              SelectedLocoChip := SuitableTrainsAtFirstStation[0];
              WorkingTimetable_LocoChip := SelectedLocoChip;
              Log('W W=' + WorkingTimetable_EntryNumStr + ': '
                     + 'No trains already in use so selected loco chip is ' + LocoChipToStr(SelectedLocoChip));
            END;
          END;
        END;

        { If none are available, see which ones are elsewhere and available, and move them to where they are needed. Create additional
          working timetable entries for these movements.
        }
        IF NOT OK AND ImportTrain THEN BEGIN
          { preserve these two variables as they are otherwise changed when new entries are created }
          FindAdditionalTrains(WorkingTimetable_EntryNumStr, StationsMaxLength, WorkingTimetableCount,
                               WorkingTimetableRecArray[WorkingTimetableCount], SuitableAdditionalTrains, OK);
          IF OK THEN BEGIN
            SelectMostSuitableAdditionalTrain(WorkingTimetable_EntryNumStr, SuitableAdditionalTrains, SelectedLocoChip, StartArea,
                                              EndArea1, EndArea2, EndArea3, Direction1, Direction2, Direction3, TravelTimeInMinutes1,
                                              TravelTimeInMinutes2, TravelTimeInMinutes3, DepartureTime1, DepartureTime2,
                                              DepartureTime3, WorkingTimetable_PreferredNumberOfCarriages);
            InsertAdditionalWorkingTimetableEntry(WorkingTimetable_EntryNumStr + 'A', SelectedLocoChip, WorkingTimetableCount,
                                                  StartArea, EndArea1, TravelTimeInMinutes1, DepartureTime1, Direction1);
            WorkingTimetable_FinalStationArea := EndArea1;
            Inc(WorkingTimetableCount);
            IF EndArea2 <> UnknownArea THEN BEGIN
              InsertAdditionalWorkingTimetableEntry(WorkingTimetable_EntryNumStr + 'B', SelectedLocoChip, WorkingTimetableCount,
                                                    EndArea1, EndArea2, TravelTimeInMinutes2, DepartureTime2, Direction2);
              WorkingTimetable_FinalStationArea := EndArea2;
              Inc(WorkingTimetableCount);
            END;

            IF EndArea3 <> UnknownArea THEN BEGIN
              InsertAdditionalWorkingTimetableEntry(WorkingTimetable_EntryNumStr + 'C', SelectedLocoChip, WorkingTimetableCount,
                                                    EndArea2, EndArea3, TravelTimeInMinutes3, DepartureTime3, Direction3);
              WorkingTimetable_FinalStationArea := EndArea3;
              Inc(WorkingTimetableCount);
            END;
          END;
        END;
      ...
      { Revise the journey using the new record(s) if inserted }
      WITH WorkingTimetableRecArray[WorkingTimetableCount] DO BEGIN
        WorkingTimetable_LocoChip := SelectedLocoChip;
        T := GetTrainRecord(WorkingTimetable_LocoChip);
        IF T <> NIL THEN BEGIN
          WITH T^ DO BEGIN
            Train_WorkingTimetableLastArrivalTime := IncMinute(WorkingTimetable_FirstStationDepartureTime,
                                                                                                  WorkingTimetable_TravelTimeInMinutes);
            Train_WorkingTimetableLastArrivalArea := WorkingTimetable_LastStationArea;
            Train_WorkingTimetableLastEntryNumStr := WorkingTimetable_EntryNumStr;

            Log(Train_LocoChip, 'W W=' + WorkingTimetable_EntryNumStr + ': '
                                   + 'Train_WorkingTimetableLastArrivalTime set to '
                                                                                    + TimeToHmStr(Train_WorkingTimetableLastArrivalTime)
                                   + '; Train_WorkingTimetableLastArrivalArea set to '
                                                                                      + AreaToStr(Train_WorkingTimetableLastArrivalArea)
                                   + '; Train_WorkingTimetableLastEntryNumStr set to ' + Train_WorkingTimetableLastEntryNumStr);
          END; {WITH}
        END;
      END; {WITH}
      Inc(WorkingTimetableCount);
    ...
    { Now arrange to move the trains back to nearest sidings etc. at the end of their working day - alter this where the trains can be
      sent back to their home area.
    }
    LastWorkingTimetableCount := High(WorkingTimetableRecArray);
    T := TrainList;
    WHILE T <> NIL DO BEGIN
      WITH T^ DO BEGIN
        { look at the Train_WorkingTimetableLastArrivalArea field, as it will be set to unknown area if the train hasn't moved }
        TempDirection1 := UnknownDirection;

        IF Train_WorkingTimetableLastArrivalArea <> UnknownArea THEN BEGIN
          IF (Train_HomeArea <> UnknownArea) AND (Train_WorkingTimetableLastArrivalArea <> Train_HomeArea) THEN BEGIN
            { arrange to move the train back to its home area }
            TempDirection1 := GetRouteDirection(Train_WorkingTimetableLastArrivalArea, Train_HomeArea);
            IF TempDirection1 <> UnknownDirection THEN BEGIN
              Log(Train_LocoChip, 'W W=' + Train_WorkingTimetableLastEntryNumStr + ': '
                                     + ' Return to home area route found between ' + AreaToStr(Train_WorkingTimetableLastArrivalArea)
                                     + ' and ' + AreaToStr(Train_HomeArea));
              TempLength := GetAreaToAreaRouteLengthInInches(Train_WorkingTimetableLastEntryNumStr,
                                                             Train_WorkingTimetableLastArrivalArea,
                                                             Train_HomeArea,
                                                             Train_TypeNum,
                                                             TempDirection1,
                                                             Train_CurrentLengthInInches DIV CarriageLengthInInches);

              TimetabledMovementTravelTimeInMinutes := GetJourneyTimeInMinutesFromRouteLength(Train_WorkingTimetableLastEntryNumStr,
                                                                                              Train_WorkingTimetableLastArrivalArea,
                                                                                              Train_HomeArea,
                                                                                              Train_TypeNum,
                                                                                              TempLength,
                                                                                              TempDirection1);
              Inc(LastWorkingTimetableCount);
              InsertAdditionalWorkingTimetableEntry(IntToStr(LastWorkingTimetableCount) + 'D', Train_LocoChip,
                                                    LastWorkingTimetableCount, Train_WorkingTimetableLastArrivalArea, Train_HomeArea,
                                                    TimetabledMovementTravelTimeInMinutes,
                                                    IncMinute(Train_WorkingTimetableLastArrivalTime,
                                                                                          StationEndOfDayPassengerLeavingTimeInMinutes),
                                                    TempDirection1);
            END ELSE BEGIN
              TempDirection2 := UnknownDirection;
              { there's no direct route - see if we can reach it by reversing once }
              FOR I := 0 TO High(ReversingAreas) DO BEGIN
                WITH ReversingAreas[I] DO BEGIN
                  IF ReversingAreas_Area <> Train_WorkingTimetableLastArrivalArea THEN BEGIN
                    TempDirection1 := GetRouteDirection(Train_WorkingTimetableLastArrivalArea, ReversingAreas_Area);
                    IF TempDirection1 <> UnknownDirection THEN BEGIN
                      TempDirection2 := GetRouteDirection(ReversingAreas_Area, Train_HomeArea);
                      IF TempDirection2 <> UnknownDirection THEN BEGIN
                        TempLength := GetAreaToAreaRouteLengthInInches(Train_WorkingTimetableLastEntryNumStr,
                                                                       Train_WorkingTimetableLastArrivalArea,
                                                                       ReversingAreas_Area,
                                                                       Train_TypeNum,
                        ...
                        TimetabledMovementTravelTimeInMinutes := GetJourneyTimeInMinutesFromRouteLength(
                                                                                                  ...
                                                                                                  TempDirection1);
                        Inc(LastWorkingTimetableCount);
                        InsertAdditionalWorkingTimetableEntry(IntToStr(LastWorkingTimetableCount) + 'D',
                                                              Train_LocoChip,
                                                              LastWorkingTimetableCount,
                                                              Train_WorkingTimetableLastArrivalArea,
                                                              Train_HomeArea,
                                                              TimetabledMovementTravelTimeInMinutes,
                                                              IncMinute(Train_WorkingTimetableLastArrivalTime,
                        ...
                        TimetabledMovementTravelTimeInMinutes := GetJourneyTimeInMinutesFromRouteLength(
                                                                                                  ...
                                                                                                  TempDirection2);
                        Inc(LastWorkingTimetableCount);
                        InsertAdditionalWorkingTimetableEntry(IntToStr(LastWorkingTimetableCount) + 'E', Train_LocoChip,
                                                              LastWorkingTimetableCount,
                                                              Train_WorkingTimetableLastArrivalArea,
                                                              Train_HomeArea,
                                                              TimetabledMovementTravelTimeInMinutes,
              ...
              IF (TempDirection1 = UnknownDirection) OR (TempDirection2 = UnknownDirection) THEN BEGIN
                { there's no direct route - see if we can reach it by reversing twice }
                debug;
              END;
            END;
          END;
          ...
          IF TempDirection1 = UnknownDirection THEN BEGIN
            { there's absolutely no route at all - best move the train to the nearest available siding/fiddleyard }
        ...
        ELSE
          ...
          ELSE BEGIN
            ...
            ELSE BEGIN
              ...
              Log('W W=' + WorkingTimetable_EntryNumStr + ': FirstStationDepartureTime='
                                                                             + TimeToHMStr(WorkingTimetable_FirstStationDepartureTime));
              ...
              Log('W W=' + WorkingTimetable_EntryNumStr + ': PreferredNumberOfCarriages='
                                                                               + IntToStr(WorkingTimetable_PreferredNumberOfCarriages));
              Log('W W=' + WorkingTimetable_EntryNumStr + ': Status=' + WorkingTimetableStatusToStr(WorkingTimetable_Status));
              Log('W W=' + WorkingTimetable_EntryNumStr + ': StoppingAtStationAreas='
                                                                             + AreaArrayToStr(WorkingTimetable_StoppingAtStationAreas));
              ...
              Log('W W=' + WorkingTimetable_EntryNumStr + ': T^.Train_WorkingTimetableLastArrivalTime='
                                                                               + TimeToHMStr(T^.Train_WorkingTimetableLastArrivalTime));
              Log('W W=' + WorkingTimetable_EntryNumStr + ': T^.Train_WorkingTimetableLastArrivalArea='
                                                                                 + AreaToStr(T^.Train_WorkingTimetableLastArrivalArea));
              DrawLineInLogFile(NoLocoChip, 'W', '-', UnitRef);
            END;
